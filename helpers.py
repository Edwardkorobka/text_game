...
def shop():
    for item in items:
        ...
        if "hp" in item:
            print(f"{item['hp']} к здоровью")
        if "attack" in item:
            print(f"{item['attack']} к атаке")
...
def inventory():
    for item in player["inventory"]:
        ...
        if item["mode"] == "equipment":
            print("Экипирован")
        if item["mode"] == "single":
            print("Одноразовое использование")
    choice = int(input("Выберите предмет(1,2,3...)"))
    if choice >= len(player["inventory"]):
        choice = len(player["inventory"])-1
    if choice < 1:
        choice = 1
    ...
    if item["mode"] == "unequipment":
    ...
    elif item["mode"] == "equipment":
        item["mode"] = "unequipment"
        if "luck" in item:
            player["luck"] -= item["luck"]
        if "armor" in item:
            player["armor"] -= item["armor"]
        ...
        if "attack" in item:
            player["attack"] -= item["attack"]
        print(f"Вы сняли {item['name']}")
    elif item["mode"] == "single":
        if "luck" in item:
            player["luck"] += item["luck"]
        if "armor" in item:
            player["armor"] += item["armor"]
